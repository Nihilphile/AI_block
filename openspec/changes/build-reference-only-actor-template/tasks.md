...
- [ ] 5.1 Extend repository boundary verification so Runtime Contracts remain schema/value-only and Actor domain/application code cannot import infrastructure, ActorHost, Host Gateway implementation, Package routing, Run Engine, Graph, or concrete Backend process code.
- [ ] 5.2 Run focused ActorTemplate/Runtime Contracts tests and the repository build, typecheck, boundary, and full verification suites; resolve only failures attributable to this change.
