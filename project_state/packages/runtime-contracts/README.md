...
The accepted Phase 0B boundary is stable and the later ActorTemplate and Project/Definition Brick application additions are additive. The Project/Definition Brick surface, including its public normalization and digest helpers, does not itself complete Project canonical-authoring remediation, establish Project persistence, or add Actor resolver integration. Host, Actor, Package, and ActorTemplate vocabulary is ahead of current workflow implementations. `model_id` is preserved in ActorTemplate snapshots but is not a first-class field in the current `ActorLaunchSpec`; launchability therefore remains a deferred cross-module decision, not a Contract defect or blocker for this card.
